...
export const buildProgressKey = (movieId, season = 0, episode = 0) => {
  return `${KEY_PREFIX}${movieId}:${normalizePart(season)}:${normalizePart(episode)}`;
};

const parseCachedProgress = (raw) => {
  if (!raw) return null;
...
export const isCompletedPosition = (position, duration) => {
...
export const getStoredProgressData = (movieId, season = 0, episode = 0) => {
...
export const getStoredProgress = (movieId, season = 0, episode = 0) => {
...
export const clearStoredProgress = (movieId, season = 0, episode = 0) => {
  if (!movieId || !isBrowser()) return;

...
export default function AnimePage() {
  ...
  // later you can use this for "load more" pages
  const [page, setPage] = useState(1);
  
  const imgURL = (path) =>
    path && path.startsWith("http") ? path : `${API_BASE}${path || ""}`;

  // ✅ LOAD ANIME DIRECTLY FROM TMDB (no Mongo)
  useEffect(() => {
    if (!API_BASE) return;

    setLoading(true);
    fetch(`${API_BASE}/api/movies/tmdb/anime?page=${page}&lang=mn`)
      .then((res) => res.json())
        .then((data) => {
        const list = Array.isArray(data.results) ? data.results : [];

        // 🧩 append new page, avoid duplicates by tmdbId
        setAnime((prev) => {
          if (page === 1) return list; // first load
        ...
        setGenres((prev) => {
          const base = new Set(prev.length ? prev : ["Anime"]);
          gSet.forEach((g) => base.add(g));
          return Array.from(base);
        });
      })
      .catch((err) => {
        console.error("TMDB anime fetch error:", err);
        setAnime([]);
      })
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setSliderIndex((prev) => (prev + 1) % anime.length);
    }, 5000);
  ...
  const handleOpenAnime = async (tmdbId) => {
    ...
    try {
      ...
      console.error("open anime error", err);
    ...
    <div className="bg-black min-h-screen text-white">
      ...
      {currentSlide && (
        <div
          ...
          // Later you’ll make a real /anime/[tmdbId] page.
          onClick={() => handleOpenAnime(currentSlide.tmdbId)}
        >
          {/* dark overlay */}
        ...
        <div className="grid grid-cols-2 gap-4 sm:flex sm:flex-wrap sm:gap-6">
          {filtered.map((m) => (
            <div
              ...
              onClick={() => handleOpenAnime(m.tmdbId)}
            >
              {/* rating badge */}
              {m.rating > 0 && (
                <div
